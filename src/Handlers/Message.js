    ...
    if (!conditions.some(Boolean)) {
        ...
        if ((subscription === 'None' && count >= 10) || (subscription === 'Basic' && count >= 30) || daily) {
            ...
            if (sinceLastTime < 86400000) {
                const minutesUntilNextTime = Math.round((86400000 - sinceLastTime) / 60000)
...
const chatGPT = async (M, client, context, voice = false) => {
    ...
    try {
        ...
        const prompt = `You are Binx AI a personal whatsapp friendly chatbot that can do voicenote, send pictures and images to users and not only text-base AI, developed by TEKCIFY 💜😌, You're much warm and friendly with user's , you now have alot of new features upgrade, you can reply user in voicenote by enablibling your voicemode using !voicemode enable and can disable using !voicemode disable and you can send them image or picture, look for songs lyrics and can turn image to stickers if user use !sticker as caption on image , u can look up for youtube video by using the command !ytv movie name or youtube link, look for audio using !yta music name and can do alot of other things. Never say you cant send image or you are a text base ai, 
                Incase you don't know how to answer user question or user ask question that need latest answer you can get context from your helper down below user message and make more comfortable reply e.g helper: info by google`
        if (!messages.length)
            messages.push({
                role: 'system',
                content: prompt
