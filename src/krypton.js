...
const start = async () => {
    ...
    app.all('/verify', async (req, res) => {
        const { phone, code, subscription } = req.method === 'GET' ? req.query : req.body
        if (![phone, code, subscription].some(Boolean)) return void res.sendStatus(404)
        if (!['Basic', 'Premium'].includes(subscription)) return res.status(404).json({ failed: 'Invaild Subscription' })
