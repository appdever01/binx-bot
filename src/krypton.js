...
const start = async () => {
    ...
    client.makeWaJid = (str) => (/\d/.test(str) ? str.replace(/\D/g, '') : 123) + '@s.whatsapp.net'
    ...
    client.isWaNumber = async (phone) => (await client.onWhatsApp(phone))[0]?.exists || false
    ...
    app.all('/request', async (req, res) => {
        ...
        if (!valid) return void res.status(404).json({ failed: 'Number not available on WhatsApp' })
        if (verification.has(jid)) {
            const storedCode = verification.get(jid)
            const remainingTime = client.utils.convertMs(storedCode.expiration - Date.now())
            return void res.status(200).json({ cooldown: `Please wait for ${remainingTime} seconds` })
        }
    ...
    app.all('/verify', async (req, res) => {
        const { phone, code, credit, key } = req.method === 'GET' ? req.query : req.body
        if (!key || key !== Key) return void res.sendStatus(403)
        if (![phone, code, credit].some(Boolean)) return void res.sendStatus(404)
       ...
       info = await client.daily.get(jid)
        info = info || {}
        const cr = parseFloat(credit) || 0 
        info.credit = (info.credit || 0) + credit
        info.count = 0
        await client.daily.set(jid, info)
        const successful = `Congratulations 🎉🎊. You have successfully added $${cr.toFixed(3)} to your balance ✅`
